...
class SiiliCameraController:
    ...
    def set_camera_source(self, camera_name, ndi_source):
        ...
        try:
            ...
            return None
    def set_camera_source_substring(self, camera_name, ndi_source):
        """Sets an NDI source based on the string given if there is such that the string is inside
        for an existing siilicam OBS source."""
        try:
            payload = {
                'obs_source_name': camera_name,
                'ndi_source': ndi_source
            }
            response = self.session.post(f'{self.base_url}/setFirstMatchingNDISource', json=payload)
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
    ...
    def set_camera_visibility(self, camera_name, visibility: bool):
        ...
        try:
            ...
            response = self.session.post(f'{self.base_url}/siiliSourceVisibility', json=payload)
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
            print(f"Error setting camera source: {e} {response.json()}")
...
if __name__ == "__main__":
    asyncio.run(set_random_ndi_source("siili"))
